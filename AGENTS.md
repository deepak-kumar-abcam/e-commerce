...
exported from the PHX REST application's Swagger endpoint. `npm run api:fetch` refreshes them all
(sorting properties and components so unchanged specs produce no diff). Never hand-edit those files —
