...
## Intershop API reference

Generated by `starlight-openapi` from the YAML files in `openapi/intershop/`, one per API, as
exported from an OpCo's REST application (`{site}/{application}`) — every OpCo sales channel serves
the same APIs from its own, and the PHX US export is the one checked in. `npm run api:fetch`
refreshes them all, `--site` and `--app` switch OpCo (sorting properties and components so
unchanged specs produce no diff). Never hand-edit those files — the next fetch overwrites them.

`src/lib/openapi.mjs` is shared by `astro.config.mjs` and the `/api/` overview page. It flags
Danaher customizations (resource class in `com.danaher` or named `Danaher…`, or a Danaher/Custom
tag) and writes render-ready copies to `.astro/openapi/` that break circular `$ref`s and dedupe
operation IDs — without that the plugin overflows the stack or silently drops pages. Fix spec
problems there, not in the source YAML.
